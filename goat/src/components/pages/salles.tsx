...
export default function Salles() {
    ...
    useEffect(() => {
        const selectedSalles = salles.filter((salle) =>
            etagesSelectionne.includes(salle.floor)
        )
    ...
    return (
        <section>
                ...
                {Etages.map((etage, index) => (
                    <div key={index} className="flex items-center space-x-2">
                        <Checkbox
                            ...
                            id={`salle-${etage}`}
                            checked={etagesSelectionne.includes(etage)}
                        />
                        <label
                            htmlFor={`salle-${etage}`}
            ...
            <div className="flex justify-center items-center gap-5 flex-wrap pt-16">
                {sallesSelectionne.map((salle, index) => (
                    <Button
                        onClick={() => {
                            setIsVisible(isVisible)
                            setEventRoomId(salle.floor)
                        }}
                        key={index}
                        size={"lg"}
                    >
                        {salle ? salle.name : "pas de salle dispo"}
                    </Button>
                ))}
            </div>
        </section>
